...
(sql-software-2 ?Software - software)
 ...
 (exe-function ?Function - function)
 (path-to ?Path - path)
...
(attacker-executes-arbitrary-code ?c4 - code ?serv4 - adminserver ?Attacker - adversary)
 ...
 (mal-response ?Response - response)
  ...
  (:action Connected-to-Database-Server
    ...
    :effect(and
        (reach-to-db ?serv2)
                                        ...
                                        ;;;;;;;;; Database Server Action for CVE-2018-1058  ;;;;;;;;
    (:action Access-to-Database-Server
    :parameters (?sql - sqlserver ?Access - accesstoken)
    :precondition(and
        (access-db ?Access)
        (sql-server ?sql)
    )
...
(:action attacker-compromised-Admin-Server
    ...
    :effect (and 
        (compromised-admin-server ?serv4)
						  ...
						  ;;;;;;;;; Compromised Database Server via DNS Server;;;;;
    ...
    (:action attacker-compromised-database-Server
    ...
    :effect (and 
        (compromised-admin-server ?serv4)
						...
						;;;;;;;;; Admin Server CVE-2020-0688 ;;;;;;;;;
      (:action attacker-connects-to-software-CVE-2020-0688
    :parameters (?serv1 - webserver ?serv4 - adminserver ?s5 - software ?Internet - connection ?Attacker - adversary)
    :precondition (and
        (connected-to-admin ?serv4)
        (compromised-web-server ?serv1)
        (admin-software-3 ?s5)
    ...
    :effect (and
      ...
      (:action attacker-executes-arbitrary-code-remotely
        ...
        (:action attacker-compromised-Admin-Server
    :parameters (?serv4 - adminserver ?c4 - code ?s5 - software ?Attacker - adversary)
    :precondition (and
       (attacker-executes-arbitrary-code-in-HTTP ?c5 ?s5 ?serv4 ?Attacker)
    )
    :effect (and 
